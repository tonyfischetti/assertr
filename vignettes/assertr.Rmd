...
provided by this package:
  ...
  * `success_report` - when success results are stored 
    ...
    success prints summary of all successful validations (when being in chan) 
  ...
  * `success_df_return` - when success results are stored 
    ...
    prints data.frame with verification results (can be used for `chain_end` or 
    single verification).
...
provided by this package:
  ...
  * `defect_report` - For single rule and defected data it displays
  short info about skipping current assertion. For `chain_end` sums
  up all skipped rules for defected data. 
  
  * `defect_df_return` - For single rule and defected data it returns
...
provided by this package:
  ...
  * `defect_append` - Attaches the assertion call info on defected 
...
You may find situation in which some rules are not idependent.
...
`assert` requires existence of `am` and `vs` columns, what is checked
previously by `verify` assertion.
In above example, we want to store info about all errors after
assertions is finished but it won't happen. No `am` column in 
data returns error not related to assertion check. As a result
code execution ends up with not handled error.

To allow such situations obligatory rules were introduced.
You create obligatory rule by adding `obligatory = TRUE` inside
assertion function.
What rule was obligatory and failed, the data is marked as defected
...
running assertion on defected data and skips the rule execution.

Below we display information about skipped rule with using `defect_report`:
```{r purl = FALSE}
mtcars_without_am <- mtcars %>% 
  dplyr::select(-am)
mtcars_without_am %>% 
...
Note: By default, all assertions in chain use `success_continue` and `error_append` functions.
It allows to continue workflow in both cases and aggregate error logs.
In some cases (i.e. when you don't want to include error log and just have some error printed),
you can force using assertion specific success/error callback in chain.
