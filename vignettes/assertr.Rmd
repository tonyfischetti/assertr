...
The solution to problem is to provide as much information as you can about
how you expect data to look up front so that any deviation from this
...
Let’s say, for example, that the R’s builtin car dataset, mtcars, was not 
builtin but rather procured from a external source than was known for making
...
This indicates that average miles per gallon for a 8 cylinder car is a lowly
...
The `assert` function takes a data frame, a predicate function, and a arbitrary
number of columns to apply to predicate function to. The predicate function
...
takes a expression, and `assert` takes a predicate and columns to apply it to.
...
This also means that it `assert` will fail sooner than `verify`, potentially
...
the predicate to *columns*, it that `assert` can't confirm assertions about
...
`assertr`'s predicates, both builtin and custom, make `assert` very powerful.
The three predicates that are built in to `assertr` (more coming soon!) are

- `not_na` - that checks if an element in not NA
- `within_bounds` - that retruns a predicate function that checks if a numeric
value falls withing the bounds of supplied, and
...
the all the IDs are 7-digits by defining the following predicate:
